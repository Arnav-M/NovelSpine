...
from novelspine.ffmpeg_path import find_ffmpeg
from novelspine.path_utils import release_after_subprocess, safe_unlink
from novelspine.audio_merge import _probe_duration_ms
from novelspine.tts_config import EDGE_CHUNK_PARALLEL
...
def _merge_audio_files(chunks: list[Path], output_path: Path) -> Path:
    ...
    if not ffmpeg:
        raise RuntimeError(
            ...
            "Reinstall NovelSpine or install ffmpeg and add it to your PATH."
        )
    ...
    def synthesize_section_detailed(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "edge-tts is not installed. Run: pip install novelspine[audiobook]"
            ) from exc
        ...
        with tempfile.TemporaryDirectory(prefix="novelspine_tts_") as tmp:
            tmp_dir = Path(tmp)
