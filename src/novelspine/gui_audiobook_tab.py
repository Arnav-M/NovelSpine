"""Audiobook tab for the NovelSpine GUI.

Voice/format settings, section selection (presets + picker dialog), and the
create-audiobook action. Mixed into NovelSpineApp (see gui.py).
"""
...
from novelspine.gui_jobs import JobCancelled
...
class AudiobookTabMixin:
    ...
    def _apply_section_preset(self, preset: str) -> None:
        from novelspine.book_structure import SectionKind
    ...
    def _refresh_sections(self) -> None:
        ...
        def work(cancel: threading.Event) -> Path:
            from novelspine.convert import ConversionCancelled, convert_pdf
    ...
    def _load_sections_from_markdown(self, markdown_path: Path, *, force: bool = False) -> None:
        from novelspine.book_structure import default_audiobook_disabled_ids, parse_book_sections
    ...
    def _refresh_voice_list(self) -> None:
        from novelspine.tts_voices import default_voice, voices_for_engine
    ...
    def _preview_voice(self) -> None:
        ...
        def work(_cancel: threading.Event) -> Path:
            ...
            cache = Path(tempfile.gettempdir()) / "novelspine_voice_previews"
