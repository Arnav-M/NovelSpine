"""FastAPI local API for the NovelSpine desktop UI."""
...
from novelspine.api import files as files_module
from novelspine.api import jobs as job_module
from novelspine.api import library as library_module
from novelspine.api import prefs as prefs_module
from novelspine.api.schemas import (
    AudiobookJobRequest,
    ChapterTextResponse,
    ConvertJobRequest,
    JobCreatedResponse,
...
app = FastAPI(title="NovelSpine API", version="1.0.0")
...
def reader_chapter_text(
    ...
    if audio_path:
        from novelspine.player import load_chapters
...
def _next_event(job_id: str) -> job_module.JobEvent | None:
    for event in jobs.iter_events(job_id, timeout=25.0):
        # Forward pings too — keeps the SSE stream alive during silent phases
        # (e.g. ffmpeg merge) so WebView2 does not drop the connection.
        return event
    job = jobs.get(job_id)
...
def preview_voice(voice: str = Query(...), engine: str = Query(default="edge")) -> dict:
    ...
    out = Path(tempfile.gettempdir()) / f"novelspine_preview_{voice}.mp3"
